...
function Audio({
    ...
    if (audio) {
      ...
      volumeLine.parentElement.onmousedown = (eDown) => {
        ...
        document.addEventListener("mousemove", (e) => {
          mouseMove(e, eDown, widthLine, volumeNow);
        });
        document.addEventListener("mouseup", (e) => {
          mouseUp(e, eDown, widthLine, volumeNow);
        });
      };
    }
  ...
  return (
    <div className="audio">
      ...
      />
      <div
        className={`player-controls-container ease-in duration-300 flex justify-between items-center  p-2`}
      >
        <div className="flex items-center  ">
          <div className="mr-2">
            <figure
              onClick={() => handleActiveList()}
              className=" cursor-pointer h-[64px] w-[64px] rounded overflow-hidden"
            >
              <img src={linkThumbnail} />
            </figure>
          </div>
          <div className="grow text-left">
            <div className="text-zinc-50 text-[12px] font-semibold">
              {title}
            </div>
        ...
        <div>
          {!activeList && <SubCanvas myAudio={myAudio} fftSize={fftSize} />}
        </div>
        <div className="player-controls-bar flex">
          <div>
            <div className="flex justify-center text-zinc-50">
              <button
                ...
                className={`${
                  status === "shuffle" && "text-[#a231da]"
                } mx-1 justify-center flex items-center w-[25px] h-[25px] rounded-full hover:bg-[#271b38]`}
              >
                <BsShuffle size="12px" />
              </button>
              <button
                onClick={() => handleControls.previous(audio, indexSong)}
              ...
              <button
                id="btn-play"
                className=" justify-center flex items-center w-[28px] h-[28px] rounded-full hover:text-[#a231da]"
              >
                <BsPlayCircle size="20px" />
              </button>
              <button
                id="btn-pause"
                className=" hidden  justify-center items-center w-[28px] h-[28px] rounded-full hover:text-[#a231da]"
              >
              ...
              <button
                onClick={() => handleControls.repeat()}
                className={`${
                  status === "repeat" && "text-[#a231da]"
                } mx-1 justify-center flex items-center w-[25px] h-[25px] rounded-full hover:bg-[#271b38]`}
              >
                <FiRepeat size="12px" />
              </button>
            ...
            <div className="text-zinc-50 flex text-[12px] grow justify-between items-center group cursor-pointer">
              ...
              <div className="h-5 flex items-center">
                <div
                  onClick={() => {}}
                  id="time-line"
                  className="w-[200px] bg-gray-700 h-[2px] mx-[2px]  group-hover:h-[3px]"
                >
                  <div
                    id="line-now"
                    className="w-0 bg-white h-[2px] group-hover:h-[3px] flex justify-end items-center"
                  >
          ...
          <div className="text-zinc-50 flex items-center group cursor-pointer ml-4">
            ...
            <div className="h-5 flex items-center">
              <div
                id="volume-line"
                className="w-[50px] bg-gray-700 h-[2px] mx-[2px]  group-hover:h-[3px]"
              >
                <div
                  id="volume-now"
                  className="w-[100%] bg-white h-[2px] group-hover:h-[3px] flex justify-end items-center"
                >
                  <div className="group-hover:h-[6px] group-hover:w-[6px] group-hover:rounded-full group-hover:bg-white"></div>
                </div>
              </div>
